...
class LinesExtractor {
  ...
  static byLineInFile (location, funcName, line) {
    try {
      const R_OK = fs.constants && fs.constants.R_OK || fs.R_OK;
      fs.accessSync (location, R_OK);
      return LinesExtractor.byLineInData (fs.readFileSync (location).toString (), funcName, line);
    } catch (ex) {
      throw ex;
    }
  }
  ...
  static byLineInData (script, funcName, line) {
    const testBody = LinesExtractor._getTestBody (script, funcName, line);
    ...
    return {
      lines: LinesExtractor._getLinesFromNode (testBody.node),
