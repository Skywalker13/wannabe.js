...
exports.byPattern = watt (function * (script, data, funcName, pattern, next) {
  return yield extract (script, data, (file) => {
    const lines = data ?
      linesExtractor.byFuncInData (data, funcName, pattern) :
      linesExtractor.byFuncInFile (file, funcName, pattern);
    return {
      lines,
      pattern
    }
...
exports.byLine = watt (function * (script, data, funcName, line, next) {
  return yield extract (script, data, (file) => {
    return data ?
      linesExtractor.byLineInData (data, funcName, line) :
      linesExtractor.byLineInFile (file, funcName, line);
  }, next);
});
