...
const rowsExtractor = require ('./rows-extractor.js');


class Wannabe {
  /**
   * Build the wannabe object.
   *
  ...
  /**
   * Inspect tje frame on the current breakpoint.
   *
  ...
  * inspect (next) {
   ...
   *
   * @param {number[]} rows - Lines where put the breakpoints.
   */
  * setBreakpoints (rows, next) {
...
watt (function * (next) {
  ...
  const rows = rowsExtractor.byFile (script, 'it', 'a.1');

  const wannabe = new Wannabe (script, 'a.1');
  yield wannabe.setBreakpoints (rows);
  const frames = yield wannabe.run ();

  Object.keys (frames).forEach ((row) => {
