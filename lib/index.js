...
const extract = watt (function * (script, data, extractor, next) {
  ...
  try {
    ...
    const testFailed = wannabe.report.failures.map ((test) => {
      return test.title;
    });
    const testPassed = wannabe.report.passes.map ((test) => {
      return test.title;
    })

    tests = extracted.tests.map ((test) => {
      if (testFailed.indexOf (test.name) !== -1) {
        return Object.assign (test, {status: 'failed'});
      }
      if (testPassed.indexOf (test.name) !== -1) {
