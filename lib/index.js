...
class Wannabe extends EventEmitter {
  ...
  _spawn (pattern, next) {
    ...
    this._mocha.on ('close', () => {
      if (output.length) {
        this._report = JSON.parse (output);
      }
    });
  ...
  * _inspectGlobals (next) {
    ...
    return globals.map ((props) => {
      return props.map ((prop) => {
        prop.type  = globalsLookup[prop.ref].type;
        prop.value = globalsLookup[prop.ref].value;
        return prop;
      })
    });
  }
  ...
  * _inspectBreak (next) {
    ...
    /* Local variables */
    // frames[0].locals.filter ((local) => !vars.hasOwnProperty (local.name)).forEach ((local) => {
    //   /* Local variables */
    //   frame.locals.push ({
    //     name:  local.name,
...
const extract = watt (function * (script, data, extractor, next) {
  ...
  try {
    ...
    /* Extract status from tests */
    const testFailed = {};
    wannabe.report.failures.forEach ((test) => {
      testFailed[test.title] = test;
    });
    const testPassed = {};
    wannabe.report.passes.forEach ((test) => {
      testPassed[test.title] = test;
    });

    tests = extracted.tests.map ((test) => {
      if (testFailed[test.name]) {
        return Object.assign (test, {
          status: 'failed',
          duration: testFailed[test.name].duration
        });
      }
      if (testPassed[test.name]) {
        return Object.assign (test, {
          status: 'passed',
          duration: testPassed[test.name].duration
        });
      }
      return test;
    });
